...
class Refiner(RefinerABC):
    def raw_issue_extractor(self, config, binary_data, policy_data=None):
        raw_data = binary_data.decode("ISO-8859-1")
        ansi_reg = re.compile(r"\x1b[^m]*m")
        raw_data = ansi_reg.sub("", raw_data)
        return [
            {
                "issue_name": "Sniper Default Report",
                "issue_type": "code[info]",
                "scan_tool": "Sniper",
                "severity": 0,
                "issue_description": raw_data,
                "key": "Sniper Default Report//code[info]",
